...
- **The site boundary is request-pinned.** ConfigOps captures the WordPress network and site identity at boot. Until site-scoped Multisite storage ships, evidence and commands fail closed whenever `switch_to_blog()` moves the request elsewhere. A pinned capture receives one durable `cross_site_write_ignored` integrity warning, no cross-site value is persisted, and temporary recovery writes restore the caller's original blog-switch stack position.
