...
| Rollback | Conflict-checked full or adapter-backed field undo for site settings; conflict-checked full-value undo for ordinary network settings additions and updates | Network delete reconstruction plus authority/lifecycle commands, named sessions, and whole-capture undo |
...
- **The network boundary is independently pinned.** Network observation is registered only when ConfigOps is network-active, requires `manage_network_options`, accepts only the current network ID, and stores its state in network options. The Network Admin ledger has separate scoped REST routes. It exposes conflict-checked mutation undo only for complete ordinary-settings additions and updates; site capture, named network sessions, whole-capture undo, redacted values, authority and plugin-lifecycle state, derived counters, and deletions remain unavailable.
...
The unreleased 0.4 line makes Network Admin evidence and narrow, mutation-level undo first-class. Complete ordinary Network Options additions and updates have network-scoped conflict, lock, audit, verification, and compensation contracts. Deletes remain review-only because WordPress reports them after the previous value is unavailable. Super-admin authority, network-wide plugin lifecycle, and derived counters need dedicated WordPress commands rather than raw option replacement. Named network sessions, whole-capture undo, cross-site aggregation, bulk operations, and fleet control require separate safety and product contracts.
