...
The command builds a pinned WordPress 7.0 / PHP 8.3 container with Xdebug, starts a temporary MariaDB 11.4 database, installs WP Mail SMTP 4.9.0, Yoast SEO 28.3, and WooCommerce 11.0.1, and then merges six independent fragments:

- unit and deterministic fuzz checks;
- adversarial input and hostile value-shape checks;
- 314 WordPress observation, persistence, concurrency, privacy, retention, agent-transport, and restore integration assertions, including capability-gated agent undo with an explicit danger acknowledgement, absent-confirmation refusal, schema 10-to-11 provenance migration, adapterless plugin activation, direct-versus-registered Settings API provenance, registration removal and malformed-hook handling, capture-time version provenance, readable nested review fields, generic array eligibility, add/remove/replace reversal, sibling preservation, target conflicts, filtered option refusal, current-parent type safety, current adapter ownership, malformed evidence, list refusal, post-write autoload compensation, REST authorization, native Ability discovery, pointer ownership races, and restore/retention exclusion;
- WP-CLI observation without an impersonated WordPress user;
- real adapter-contract checks, including the option maps, registered defaults, and Settings API fields published by each supported plugin release;
- network-active Multisite and Multi-Network observation, virtualized Network Options refusal, mutation undo, audit, locking, REST authorization, scope isolation, retention, large-network behavior, lifecycle, migration, site-deletion, and uninstall checks.
